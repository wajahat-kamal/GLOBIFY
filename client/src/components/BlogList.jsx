...
function BlogList() {
  ...
  return (
    <section className="w-full px-4 md:px-10 py-6 pt-8">
      {/* Category Buttons */}
      <div className="flex flex-wrap items-center justify-center md:gap-6 gap-2 mb-5">
        {blogCategory.map((item) => (
          <button
            key={item}
            onClick={() => setMenu(item)}
            className={`relative px-4 py-1 text-sm md:text-[17px] font-medium rounded-4xl overflow-hidden
